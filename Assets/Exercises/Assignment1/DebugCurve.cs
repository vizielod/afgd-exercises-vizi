...
namespace AfGD
{
    ...
    public class DebugCurve : MonoBehaviour
    {
        ...
        bool Init()
        {
            ...
            if (curves == null || curves.Length != points)
                curves = (points == 0) ? new CurveSegment[0] : new CurveSegment[points];

            for (int i = 0; i < points; i++)
            {
                //Instantiate(myPrefab_CP, path[i], Quaternion.identity);
                GameObject temp = Instantiate(myPrefab_CP, path[i], Quaternion.identity);
                controlPoints.SetValue(temp.transform, i);
            }
